...
We have just finished making some changes in this repository. Your task is to review the existing documentation in `docs/` and update it to reflect the changes in the code, so that future agents can easily orient themselves in this codebase.
...
- The only exception is the single, centralized `utils.md` file, which is a pointer file to existing utils/helpers which allow future agents to find and re-use these easily.
